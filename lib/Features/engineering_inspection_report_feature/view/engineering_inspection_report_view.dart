...
import '../data/services/engineering_inspection_report_api_service.dart';
import 'widgets/certificate_installation_form.dart';
import 'widgets/success_dialog.dart';

class EngineeringInspectionReportView extends StatefulWidget {
  const EngineeringInspectionReportView({super.key});

  @override
  State<EngineeringInspectionReportView> createState() =>
      _ServiceProviderSelectionViewState();
}

class _ServiceProviderSelectionViewState
    extends State<EngineeringInspectionReportView> {
  final _formKey = GlobalKey<FormState>();
  final _areaController = TextEditingController();
  final _certificateApiService = EngineeringInspectionReportApiService();
  ...
  void _handleSubmit(AppLocalizations localizations) async {
    if (_formKey.currentState?.validate() == true) {
      ...
      final hasDevices = _alarmItems.any((device) => device.count > 0) ||
          _fireSystemItems.any((extinguisher) => extinguisher.count > 0);
      ...
      if (area > 0 && hasDevices) {
        ...
        final request = CertificateInstallationRequest(
          ...
          requestType: 'EngineeringInspection',
